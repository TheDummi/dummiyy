...
(() => {
	...
	var je = (t, e, n, r) => {
		if ((e && typeof e == 'object') || typeof e == 'function') for (let i of Ne(e)) !Be.call(t, i) && i !== n && le(t, i, { get: () => e[i], enumerable: !(r = Fe(e, i)) || r.enumerable });
		return t;
	};
	var $e = (t, e, n) => ((n = t != null ? De(Ve(t)) : {}), je(e || !t || !t.__esModule ? le(n, 'default', { value: t, enumerable: !0 }) : n, t));
	var pe = qe((de, he) => {
		(function () {
			var t = function (e) {
				var n = new t.Builder();
...
`,
			...
			(t.Vector = function (e) {
				...
				(t.Vector.prototype.dot = function (e) {
					for (var n = 0, r = this.elements, i = e.elements, s = r.length, o = i.length, a = 0, l = 0, c = 0, d = 0; c < s && d < o; )
						(a = r[c]), (l = i[d]), a < l ? (c += 2) : a > l ? (d += 2) : a == l && ((n += r[c + 1] * i[d + 1]), (c += 2), (d += 2));
					return n;
				}),
				...
				(t.Vector.prototype.toArray = function () {
					for (var e = new Array(this.elements.length / 2), n = 1, r = 0; n < this.elements.length; n += 2, r++) e[r] = this.elements[n];
					return e;
				}),
			...
			(t.stemmer = (function () {
				var e = {
					...
					F = /^(.+?[^aeiou])y$/,
					j = /^(.+?)(ational|tional|enci|anci|izer|bli|alli|entli|eli|ousli|ization|ation|ator|alism|iveness|fulness|ousness|aliti|iviti|biliti|logi)$/,
					...
					V = function (u) {
						...
						if (
							...
							var E = x.exec(u);
							(g = E[1]), (x = v), x.test(g) && ((u = g), (x = k), (O = _), (R = q), x.test(u) ? (u = u + 'e') : O.test(u) ? ((h = w), (u = u.replace(h, ''))) : R.test(u) && (u = u + 'e'));
						}
						if (((h = F), h.test(u))) {
							var E = h.exec(u);
							(g = E[1]), (u = g + 'i');
						...
						return (h = W), (x = p), h.test(u) && x.test(u) && ((h = w), (u = u.replace(h, ''))), T == 'y' && (u = T.toLowerCase() + u.substr(1)), u;
					};
				return function (M) {
					return M.update(V);
				};
			...
			(t.TokenSet = function () {
					...
					for (var r = new t.TokenSet(), i = [{ node: r, editsRemaining: n, str: e }]; i.length; ) {
						...
						if (s.editsRemaining != 0) {
							...
							if (
								...
								s.str.length == 1 && (c.final = !0), i.push({ node: c, editsRemaining: s.editsRemaining - 1, str: s.str.slice(1) });
							}
							if (s.str.length > 1) {
				...
				(t.TokenSet.prototype.intersect = function (e) {
					for (var n = new t.TokenSet(), r = void 0, i = [{ qNode: e, output: n, node: this }]; i.length; ) {
						r = i.pop();
						for (var s = Object.keys(r.qNode.edges), o = s.length, a = Object.keys(r.node.edges), l = a.length, c = 0; c < o; c++)
							for (var d = s[c], m = 0; m < l; m++) {
								...
								if (p == d || d == '*') {
									var b = r.node.edges[p],
										...
										L = b.final && v.final,
										f = void 0;
									p in r.output.edges ? ((f = r.output.edges[p]), (f.final = f.final || L)) : ((f = new t.TokenSet()), (f.final = L), (r.output.edges[p] = f)),
			...
			(t.Index = function (e) {
				...
				(t.Index.prototype.query = function (e) {
					for (
						var n = new t.Query(this.fields), r = Object.create(null), i = Object.create(null), s = Object.create(null), o = Object.create(null), a = Object.create(null), l = 0;
							...
							for (var S = 0; S < L.length; S++)
								for (var w = L[S], k = this.invertedIndex[w], _ = k._index, f = 0; f < c.fields.length; f++) {
									var y = c.fields[f],
										...
										j = w + '/' + y,
										$ = new t.Set(F);
									if ((c.presence == t.Query.presence.REQUIRED && ((m = m.union($)), o[y] === void 0 && (o[y] = t.Set.complete)), c.presence == t.Query.presence.PROHIBITED)) {
				...
				(t.Index.prototype.toJSON = function () {
					...
					return { version: t.version, fields: this.fields, fieldVectors: n, invertedIndex: e, pipeline: this.pipeline.toJSON() };
				}),
			...
			(t.Builder = function () {
						...
						for (var b = 0; b < d.length; b++) {
							...
							for (var y = 0; y < this.metadataWhitelist.length; y++) {
								var S = this.metadataWhitelist[y],
									w = v.metadata[S];
								this.invertedIndex[v][o][r][S] == null && (this.invertedIndex[v][o][r][S] = []), this.invertedIndex[v][o][r][S].push(w);
							}
						}
					}
				}),
				...
				(t.Builder.prototype.createFieldVectors = function () {
					for (var e = {}, n = Object.keys(this.fieldTermFrequencies), r = n.length, i = Object.create(null), s = 0; s < r; s++) {
						for (
							...
							i[f] === void 0 ? ((w = t.idf(this.invertedIndex[f], this.documentCount)), (i[f] = w)) : (w = i[f]),
								(k = (w * ((this._k1 + 1) * y)) / (this._k1 * (1 - this._b + this._b * (l / this.averageFieldLength[a])) + y)),
								(k *= b),
								(k *= v),
				...
				(t.Builder.prototype.build = function () {
					return (
						...
						this.createTokenSet(),
						new t.Index({ invertedIndex: this.invertedIndex, fieldVectors: this.fieldVectors, tokenSet: this.tokenSet, fields: Object.keys(this._fields), pipeline: this.searchPipeline })
					);
				}),
				...
				(function (e, n) {
					typeof define == 'function' && define.amd ? define(n) : typeof de == 'object' ? (he.exports = n()) : (e.lunr = n());
				})(this, function () {
					return t;
				});
		})();
	...
	var J = class {
		...
		showPage() {
			document.body.style.display && (document.body.style.removeProperty('display'), this.ensureFocusedElementVisible(), this.updateIndexVisibility(), this.scrollToHash());
		}
		scrollToHash() {
			if (location.hash) {
		...
		ensureFocusedElementVisible() {
			if (
				(this.alwaysVisibleMember && (this.alwaysVisibleMember.classList.remove('always-visible'), this.alwaysVisibleMember.firstElementChild.remove(), (this.alwaysVisibleMember = null)),
				!location.hash)
			)
				return;
			let e = document.getElementById(location.hash.substring(1));
	...
	function ve() {
		...
		if (!t) return;
		let e = { base: document.documentElement.dataset.base + '/' },
	...
	function We(t, e, n, r) {
		n.addEventListener(
			...
			n.addEventListener('keydown', (i) => {
				i.key == 'Enter' ? Je(e, t) : i.key == 'ArrowUp' ? (me(e, n, -1), i.preventDefault()) : i.key === 'ArrowDown' && (me(e, n, 1), i.preventDefault());
			}),
			...
			document.body.addEventListener('keyup', (i) => {
				t.classList.contains('has-focus') && (i.key === 'Escape' || (!e.matches(':focus-within') && !n.matches(':focus'))) && (n.blur(), re(t));
			});
	}
	function re(t) {
	...
	function Ue(t, e, n, r) {
		...
		for (let o = 0; o < s.length; o++) {
			let a = s[o],
				...
				c = 1;
			l.name.toLowerCase().startsWith(i.toLowerCase()) && (c *= 1 + 1 / (1 + Math.abs(l.name.length - i.length))), (a.score *= c);
		}
		if (s.length === 0) {
	...
	function me(t, e, n) {
		...
		if (!r) (r = t.querySelector(n == 1 ? 'li:first-child' : 'li:last-child')), r && r.classList.add('current');
			...
			if (n === 1)
				...
				while (i instanceof HTMLElement && i.offsetParent == null);
			i ? (r.classList.remove('current'), i.classList.add('current')) : n === -1 && (r.classList.remove('current'), e.focus());
		}
	}
	function Je(t, e) {
	...
	function ye(t, e) {
		...
		let n = t.toLocaleLowerCase(),
			...
			s = 0,
			o = n.indexOf(r);
		for (; o != -1; ) i.push(ne(t.substring(s, o)), `<b>${ne(t.substring(o, o + r.length))}</b>`), (s = o + r.length), (o = n.indexOf(r, s));
	...
	Le && 'ontouchstart' in document.documentElement && ((Xe = !0), (A = 'touchstart'), (Ee = 'touchmove'), (B = 'touchend'));
...
`),
		...
		handleValueChange() {
			(this.el.checked = this.value), document.documentElement.classList.toggle(this.key, this.value), this.app.filterChanged(), this.app.updateIndexVisibility();
		}
	};
	var oe = new Map(),
		ae = class {
	...
	function Ie(t, e, n) {
		...
		if (t.children) {
			...
			for (let c of t.children) Ie(c, l, i);
		} else ke(t, r, t.class);
	}
	function ke(t, e, n) {
		if (t.path) {
	...
	async function Qe() {
		...
		if ((i.classList.add('tsd-hierarchy'), Ke(i, n, e), r.querySelectorAll('li').length == i.querySelectorAll('li').length)) return;
	...
	function Oe(t, e, n, r = new Set()) {
		...
		if ((s.classList.add('tsd-hierarchy-item'), e === n)) {
			...
			for (let a of i.uniqueNameParents || []) {
				let l = t.reflections[a],
					c = s.appendChild(document.createElement('a'));
				(c.textContent = l.name), (c.href = te + l.url), (c.className = l.class + ' tsd-signature-type'), s.append(document.createTextNode('.'));
			}
